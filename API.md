...
## L.bigemap.map(element, id|url|tilejson, options)
...
| id _or_ url _or_ tilejson | __string__ if _id_ or _url_ __object__ if _tilejson_ | url can be <ul><li>A map `id` string `examples.map-foo`</li><li>A comma separated list of map `id` strings `examples.map-foo,examples.map-bar` [example](https://www.bigemap.com/bigemap.js/example/v1.0.0/compositing/)</li><li>A URL to TileJSON, like `{{site.tileApi}}/v3/bigemap.dark.json`</li><li>A [TileJSON](https://www.bigemap.com/developers/tilejson/) object, from your own Javascript code</li></ul> |
| options | object | If provided, it is the same options as provided to L.Map with the following additions: <ul><li>`tileLayer` L.TileLayer options. Options passed to a `L.bigemap.tileLayer` based on the TileJSON. Set to `false` to disable the `L.bigemap.tileLayer`.</li><li>`featureLayer` `L.bigemap.featureLayer` options. Options passed to a `L.bigemap.featureLayer` based on the TileJSON. Set to `false` to disable the `L.bigemap.featureLayer`.</li><li>`gridLayer` `L.bigemap.gridLayer`. Options passed to a `L.bigemap.gridLayer` based on the TileJSON. Set to `false` to disable the `L.bigemap.gridLayer`.</li><li>`legendControl` `L.bigemap.legendControl` options. Options passed to a `L.bigemap.legendControl` based on the TileJSON. Set to `false` to disable the `L.bigemap.legendControl`.</li><li>`shareControl`: Options passed to a `L.bigemap.shareControl`. Set to `true` to enable the `L.bigemap.shareControl`.</li><li>`accessToken`: Bigemap API access token. Overrides `L.bigemap.accessToken` for this map.</li><li>`attributionControl`: value can be `{compact: true}` to force a compact attribution icon that shows the full attribution on click, or `{compact: false}` to force the full attribution control. The default is a responsive attribution that collapses when the map is less than 640 pixels wide.</li> |
...
## L.bigemap.tileLayer(id|url|tilejson, options)
...
| Options | Value | Description |
| ---- | ---- | ---- |
| id _or_ url _or_ tilejson (_required_) | __string__ if _id_ or _url_ __object__ if _tilejson_ | Value must be <ul><li>An `id` string `examples.map-foo`</li><li>A URL to TileJSON, like `{{site.tileApi}}/v3/bigemap.dark.json`</li><li>A TileJSON object, from your own Javascript code</li></ul> |
| options | object | The second argument is optional. If provided, it is the same options as provided to `L.TileLayer`, with the following addition: <ul><li>`accessToken`: Bigemap API access token. Overrides `L.bigemap.accessToken` for this layer.</li></ul> |
...
_Example_:
    ...
    var layer = L.bigemap.tileLayer('{{site.tileApi}}/v3/bigemap.dark.json');

_Returns_ a `L.bigemap.tileLayer` object.

_Class_: `L.bigemap.TileLayer`
...
[Live example of .setFormat in use](https://www.bigemap.com/bigemap.js/example/v1.0.0/tilelayer-setformat/)

_Returns_: the layer object

## L.bigemap.gridLayer(id|url|tilejson, options)

An `L.bigemap.gridLayer` loads [UTFGrid](http://bigemap.com/developers/utfgrid/) tiles of
interactivity into your map, which you can easily access with `L.bigemap.gridControl`.

| Options | Value | Description |
| ---- | ---- | ---- |
| id _or_ url _or_ tilejson (_required_) | __string__ if _id_ or _url_ __object__ if _tilejson_ | <ul><li>An `id` string `examples.map-foo`</li><li>A URL to TileJSON, like `{{site.tileApi}}/v3/bigemap.dark.json`</li><li>A TileJSON object, from your own Javascript code</li></ul> |
| options | Object | The second argument is optional. If provided, it may include: <ul><li>`accessToken`: Bigemap API access token. Overrides `L.bigemap.accessToken` for this layer.</li></ul> |

_Example_:

    // the second argument is optional
    var layer = L.bigemap.gridLayer('bigemap.light');

_Returns_ a `L.bigemap.gridLayer` object.

_Class_: `L.bigemap.GridLayer`

### gridLayer.on(event, handler, context)

Bind an event handler to a given event on this `L.bigemap.gridLayer` instance.
...
_Returns_: the L.bigemap.gridLayer object

## L.bigemap.featureLayer(id|url|geojson, options)
...
| Options | Value | Description |
| ---- | ---- | ---- |
| id _or_ url _or_ geojson | __string__ if _id_ or _url_ __object__ if _tilejson_ | Must be either <ul><li>An id string bigemap.streets</li><li>A URL to TileJSON, like `{{site.tileApi}}/v3/bigemap.dark.json`</li><li>A GeoJSON object, from your own Javascript code</li><li>`null`, if you wish to only provide `options` and not initial data.</li></ul> |
| options | object | If provided, it is the same options as provided to `L.FeatureGroup`, as well as: <ul><li>`filter`: A function that accepts a feature object and returns `true` or `false` to indicate whether it should be displayed on the map. This can be changed later using `setFilter`.</li><li>`sanitizer`: A function that accepts a string containing tooltip data, and returns a sanitized result for HTML display. The default will remove dangerous script content, and is recommended.</li><li>`accessToken`: Bigemap API access token. Overrides `L.bigemap.accessToken` for this layer.</li><li>`popupOptions`: an object of <a href="http://leafletjs.com/reference.html#popup-maxwidth">options that will be passed to the `bindPopup` method internally</a>.<li>`pointToLayer`: A function that accepts a feature object and a `L.LatLng` and returns a layer to be added. Defaults to `L.bigemap.marker.style`.</li></ul> |

_Example_:

    var featureLayer = L.bigemap.featureLayer(geojson)
        .addTo(map);

_Returns_ a `L.bigemap.featureLayer` object.

_Class_: `L.bigemap.FeatureLayer`
...
_Example_:
    ...
    var featureLayer = L.bigemap.featureLayer()
        .addTo(map);
...
Load marker GeoJSON data from a map with the given `id` on Bigemap.
...
_Example_:
    ...
    // loads markers from the map `bigemap.dark` on Bigemap,
    // if that map has markers
    featureLayer.loadID('bigemap.dark');
...
[See a live example of .setFilter](https://www.bigemap.com/bigemap.js/example/v1.0.0/multiple-marker-filters/)
...
## L.bigemap.styleLayer(url, options)

<span class='leaflet icon'>_Extends_: `L.tileLayer`</span>

`L.bigemap.styleLayer` renders Bigemap Studio [styles](https://www.bigemap.com/help/define-style/) as raster tiles using the [Bigemap Static API tiles endpoint](https://www.bigemap.com/api-documentation/#retrieve-raster-tiles-from-styles).

| Options | Value | Description |
| ---- | ---- | ---- |
| url | string | Must be a string like `bigemap://styles/bigemap/cin286r4x006safncofpcb71v`|
...
_Example_:
    ...
    var styleLayer = L.bigemap.styleLayer(url)
        .addTo(map);

_Returns_ a `L.bigemap.styleLayer` object.

# Geocoding

## L.bigemap.geocoder(id|url, options)

A low-level interface to [the Bigemap Geocoding API](https://www.bigemap.com/api-documentation/#geocoding),
useful for complex uses and reverse-geocoding.

| Options | Value | Description |
| ---- | ---- | ---- |
| id _or_ url | string | Value must be <ul><li>A [geocoder index ID](https://www.bigemap.com/developers/api/geocoding/), e.g. `bigemap.places`</li><li>A geocoder API URL, like `{{site.tileApi}}/geocoding/v5/bigemap.places/{query}.json`</li></ul> |
| options | Object | The second argument is optional. If provided, it may include: <ul><li>`accessToken`: Bigemap API access token. Overrides `L.bigemap.accessToken` for this geocoder.</li></ul> |

_Returns_ a `L.bigemap.geocoder` object.

### geocoder.query(queryString|options, callback)

Queries the geocoder with a query string, and returns its result, if any.
This performs [forward geocoding](https://www.bigemap.com/api-documentation/#search-for-places).
...
_Example_: [Live example of geocoder.query centering a map.](https://www.bigemap.com/bigemap.js/example/v1.0.0/map-center-geocoding/)
...
This performs [reverse geocoding](https://www.bigemap.com/api-documentation/#retrieve-places-near-a-location).
...
| callback (_required_) | function | The callback is called with arguments <ul><li>An error, if any</li><li>The result. This is an object of the raw result from Bigemap.</li></ul>

_Returns_: the geocoder object. The return value of this function is not useful - you must use a callback to get results.

# Controls

## L.bigemap.legendControl(options)
...
_Returns_: a `L.bigemap.legendControl` object.

_Class_: `L.bigemap.LegendControl`
...
## L.bigemap.gridControl(layer, options)

<span class='leaflet icon'>_Extends_: `L.Control`</span>

Interaction is what we call interactive parts of maps that are created with the powerful [tooltips &amp; regions](http://bigemap.com/tilemill/docs/crashcourse/tooltips/) system in [TileMill](http://bigemap.com/tilemill/). Under the hood, it's powered by the open [UTFGrid specification](https://github.com/bigemap/utfgrid-spec/).

| Options | Value | Description |
| ---- | ---- | ---- |
| layer | `L.bigemap.gridLayer` | The first argument must be a layer created with `L.bigemap.gridLayer()` |
...
_Example_:
    ...
    var map = L.bigemap.map('map').setView([38, -77], 5);
    var gridLayer = L.bigemap.gridLayer('bigemap.light');
    map.addLayer(L.bigemap.tileLayer('bigemap.outdoors'));
    map.addLayer(gridLayer);
    map.addControl(L.bigemap.gridControl(gridLayer));

_Returns_: a `L.bigemap.gridControl` object.

_Class_: `L.bigemap.GridControl`

### gridControl.hide()

If a tooltip is currently shown by the gridControl, hide and close it.

_Returns_: the `L.bigemap.gridControl` object.
...
## L.bigemap.geocoderControl(id|url, options)

Adds geocoder functionality as well as a UI element to a map. This uses
the [Bigemap Geocoding API](http://bigemap.com/developers/api/geocoding/).

| Options | Value | Description |
| ---- | ---- | ---- |
| id _or_ url (_required_) | string | Either a <ul><li>An [geocoder index ID](https://www.bigemap.com/developers/api/geocoding/), e.g. `bigemap.places`</li><li>A geocoder API URL, like `{{site.tileApi}}/geocoding/v5/bigemap.places/{query}.json`</li></ul> |
| options | object | An options argument with the same options as the `L.Control` class, as well as: <ul><li>`keepOpen`: a boolean for whether the control will stay open always rather than being toggled. Default `false`. See <a href='https://www.bigemap.com/bigemap.js/example/v1.0.0/geocoder-keep-open/'>live example</a>.<li><li>`accessToken`: Bigemap API access token. Overrides `L.bigemap.accessToken` for this control.</li><li>`autocomplete`: automatically search and show results as you type. Default: `false`.</ul> |
...
_Example_:
    ...
    var map = L.map('map')
        .setView([37, -77], 5)
        .addControl(L.bigemap.geocoderControl('bigemap.places'));

_Returns_ a `L.bigemap.geocoderControl` object.

_Class_: `L.bigemap.GeocoderControl`
...
## L.bigemap.shareControl(id|url, options)
...
| Options | Value | Description |
| ---- | ---- | ---- |
| id _or_ url _optional_ | string | Either a <ul><li><code>id</code> string <code>examples.map-foo</code></li><li>A URL to TileJSON, like <code>{{site.tileApi}}/v3/bigemap.dark.json</code> If not supplied, the TileJSON from the map is used.</li></ul> |
| options | object | Options for L.Control</span> Also accepts the following options:<ul><li>`url`: the <code>URL</code> of a page to which the share control will link instead of the URL of the current page or that specified in TileJSON data.</li><li>`accessToken`: Bigemap API access token. Overrides `L.bigemap.accessToken` for this control.</li></ul> |

_Example_:

    var map = L.map('map', 'bigemap.streets')
        .setView([37, -77], 5)
        .addControl(L.bigemap.shareControl());

_Returns_:
Returns a `L.bigemap.shareControl` object.

# Markers

## L.bigemap.marker.icon(properties)
...
[A working example of L.bigemap.marker.icon in use](https://www.bigemap.com/bigemap.js/example/v1.0.0/l-bigemap-marker/)

## L.bigemap.marker.style(feature, latlng)

An icon generator for use in conjunction with `pointToLayer` to generate
markers from the [Bigemap Markers API](https://www.bigemap.com/api-documentation/#retrieve-a-standalone-marker)
and support the [simplestyle-spec](https://github.com/bigemap/simplestyle-spec) for
features.
...
The other sections of the [simplestyle-spec](https://github.com/bigemap/simplestyle-spec) are implemented
by `L.bigemap.simplestyle.style`

## L.bigemap.simplestyle.style(feature)
...
_Examples_:
    ...
    L.geoJson(geoJson, {
        pointToLayer: L.bigemap.simplestyle.style,
    });

[A working example of L.bigemap.simplestyle in use](https://www.bigemap.com/bigemap.js/example/v1.0.0/geojson-simplestyle/)
...
## L.bigemap.sanitize(string)

A HTML sanitization function, with the same effect as the default value of the `sanitizer` option of `L.bigemap.featureLayer`, `L.bigemap.gridControl`, and `L.bigemap.legendControl`.

| Options | Value | Description |
| ---- | ---- | ---- |
| text | string | String of content you wish to sanitize. |

## L.bigemap.template(template, data)

A [mustache](http://mustache.github.io/) template rendering function, as used by the templating feature provided by `L.bigemap.gridControl`.
...
_Example_:
    ...
    var output = L.bigemap.template('Name: {% raw %}{{name}}{% endraw %}', {name: 'John'});
    // output is "Name: John"

# Configuration

## L.bigemap.accessToken

The API access token to be used by Bigemap.js. You must set this value as described
in [API access tokens](../api-access-tokens).

## L.bigemap.config.FORCE_HTTPS

By default, this is `false`. Bigemap.js auto-detects whether the page your map
is embedded in is using HTTPS or SSL, and matches: if you use HTTPS on your site,
it uses HTTPS resources.

Setting `FORCE_HTTPS` to `true` makes Bigemap.js always require HTTPS resources,
...
## L.bigemap.config.HTTP_URL

A base URL from which Bigemap.js will load TileJSON and other resources. By default,
this points to the [Bigemap Web Services](https://www.bigemap.com/developers/api/).

## L.bigemap.config.HTTPS_URL

The same as `L.bigemap.config.HTTP_URL`, but used when SSL mode is detected or
...
Bigemap.js uses the Bigemap web services API, which requires an API access token. You must
supply an access token to Bigemap.js:

 ```
 L.bigemap.accessToken = '<your access token>';
 ```

To obtain an access token, sign in to Bigemap and visit the [Account Apps](https://www.bigemap.com/account/apps/)
page. For Bigemap.js, a "Public" token (starting with "pk") is required.
...
option when creating Bigemap.js objects. For example:

 ```
 var map = L.bigemap.map('map', 'bigemap.outdoors', {
   accessToken: '<your access token>'
 });
 ```

For additional help, see ["How do I create an API access token?"](https://www.bigemap.com/help/create-api-access-token/).

## Upgrading from Bigemap.js v1

In a few cases, you may need to make changes to code written for Bigemap.js 1.x
versions in order to work with Bigemap.js 2.x.

* Bigemap.js 2.x uses version 4 of the Bigemap web services API, which requires API access tokens.
You must supply an access token to Bigemap.js; see [API access tokens](../api-access-tokens) for
details.

* The `markerLayer` alias has been removed from `L.bigemap.map`. Use `featureLayer`
...
* `L.bigemap.geocoder` and `L.bigemap.geocoderControl` no longer accept arbitrary map IDs.
Instead you must provide a predefined geocoder index ID (or the ID of a custom geocoder
index). For instance, replace

 ```
 L.bigemap.geocoderControl('bigemap.outdoors').addTo(map);
 ```

 with

 ```
 L.bigemap.geocoderControl('bigemap.places').addTo(map);
 ```

 See [the geocoding API documentation](https://www.bigemap.com/developers/api/geocoding/)
 for a complete list of predefined geocoding indexes.

* The format for `L.bigemap.geocoder` and `L.bigemap.geocoderControl` results have changed.
Results are now provided in GeoJSON format. If your code uses `L.bigemap.geocoder` or
the `found`, `select`, or `autoselect` events from `L.bigemap.geocoderControl`, it may
need to be updated to expect the format documented in those classes.

* `L.bigemap.config.HTTP_URLS` and `L.bigemap.config.HTTPS_URLS` have been replaced
with `L.bigemap.config.HTTP_URL` and `L.bigemap.config.HTTPS_URL`, which expect to
be assigned a single URL rather than an array of URLs. For example, replace

 ```
 L.bigemap.config.HTTP_URLS = ["http://example.com/"];
 ```

 with

 ```
  L.bigemap.config.HTTP_URL = "http://example.com/";
 ```

* `L.bigemap.tileLayer` no longer supports `detectRetina`, `retinaVersion`, or `autoscale` options.
Instead, retina tiles are always automatically used when available.

* `L.bigemap.geocoder` no longer has `setURL`, `setID`, and `setTileJSON` methods. Instead
...
Bigemap.js is optimized for mobile devices and small screens by default.
...
Bigemap.js implements a simple, light style on all interaction elements. A dark theme
...
By default, Bigemap.js includes a bundled version of Leaflet that Bigemap has ensured
is compatible. A standalone version of Bigemap.js is also available which you can use if you would like to supply your own version of
Leaflet. When using this technique, you will use the newest version of Bigemap.css.


    <script src='{{site.tileApi}}/bigemap.js/{{site.bigemapjs}}/bigemap.standalone.js'></script>
