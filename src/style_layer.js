...
var StyleLayer = L.TileLayer.extend({
    ...
    _getAttribution: function(_) {
        ...
        request(styleURL, L.bind(function(err, style) {
            if (err) {
                util.log('could not load Bigemap style at ' + styleURL);
                this.fire('error', {error: err});
            }
            var sources = [];
            for (var id in style.sources) {
                var source = style.sources[id].url.split('bigemap://')[1];
                sources.push(source);
            }
    ...
    _formatTileURL: function(style) {
        if (typeof style === 'string') {
            if (style.indexOf('bigemap://styles/') === -1) {
                util.log('Incorrectly formatted Bigemap style at ' + style);
                this.fire('error');
            }
            var ownerIDStyle = style.split('bigemap://styles/')[1];
