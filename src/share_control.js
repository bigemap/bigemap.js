...
var ShareControl = L.Control.extend({
    ...
    onAdd: function(map) {
        ...
        var container = L.DomUtil.create('div', 'leaflet-control-bigemap-share leaflet-bar');
        var link = L.DomUtil.create('a', 'bigemap-share bigemap-icon bigemap-icon-share', container);
        link.href = '#';

        this._modal = L.DomUtil.create('div', 'bigemap-modal', this._map._container);
        this._mask = L.DomUtil.create('div', 'bigemap-modal-mask', this._modal);
        this._content = L.DomUtil.create('div', 'bigemap-modal-content', this._modal);
    ...
    _shareClick: function(e) {
        ...
        this._sharing = L.DomUtil.create('div', 'bigemap-modal-body', this._content);

        var twitterButton = createShareButton('bigemap-button bigemap-button-icon bigemap-icon-twitter', twitterURL, 'Twitter');
        var facebookButton = createShareButton('bigemap-button bigemap-button-icon bigemap-icon-facebook', facebookURL, 'Facebook');
        var pinterestButton = createShareButton('bigemap-button bigemap-button-icon bigemap-icon-pinterest', pinterestURL, 'Pinterest');
        ...
        shareButtons.setAttribute('class', 'bigemap-share-buttons');
        ...
        var input = L.DomUtil.create('input', 'bigemap-embed', this._sharing);
        input.type = 'text';
        input.value = embedValue;

        var label = L.DomUtil.create('label', 'bigemap-embed-description', this._sharing);
