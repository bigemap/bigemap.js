...
output.write('description: Build anything with Bigemap.js ' + argv.t + ', a library for fast & interactive maps.\n');
landOutput.write('description: Build anything with Bigemap.js ' + argv.t + ', a library for fast & interactive maps.\n');

output.write('permalink: /api/' + argv.t + '/all/\n');
landOutput.write('permalink: /api/' + argv.t + '/\n');

var BASE_URL = '/bigemap.js/api/' + argv.t + '/';
...
function escapeFn(text) {
    // Special case for https://github.com/bigemap/bigemap.js/issues/661
    if (text === 'L.Map') {
        return "l-map-class";
    } else {
...
function readDocumentation(filename) {
    ...
    if (filename.match(/html$/)) {
        ...
        chunks.forEach(function(c) {
            ...
            // remove extra tabs: https://github.com/bigemap/bigemap.js/issues/804
            c.text = c.text.join('\n').replace(/\t{2,}/g, '');
