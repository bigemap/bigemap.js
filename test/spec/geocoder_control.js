describe('L.bigemap.geocoderControl', function() {
    'use strict';
    var server;
    ...
    it('performs forward geolocation, centering the map on the first result', function() {
        var map = new L.Map(document.createElement('div')),
            control = L.bigemap.geocoderControl('bigemap.places', { proximity: false }).addTo(map);

        expect(control instanceof L.bigemap.GeocoderControl).to.eql(true);
    ...
    it('performs forward geolocation, centering the map on the first result even if a point', function() {
        ...
        server.respondWith('GET',
            'http://a.tiles.bigemap.com/geocoding/v5/bigemap.places/white%20house.json?access_token=key',
    ...
    it('supports the pointzoom option for preferred zoom for point results', function() {
        var map = new L.Map(document.createElement('div')),
            control = L.bigemap.geocoderControl('bigemap.places', {
                proximity: false,
                pointZoom: 10
            }).addTo(map);
    ...
    it('supports queryOptions including autocomplete and country', function() {
        ...
        server.respondWith('GET',
            'http://a.tiles.bigemap.com/geocoding/v5/bigemap.places/white%20house.json?access_token=key&country=us&autocomplete=false',
            [200, { "Content-Type": "application/json" }, JSON.stringify(helpers.geocoderWhiteHouse)]);
    ...
    it('sets url based on an id', function() {
        var control = L.bigemap.geocoderControl('bigemap.places');
        expect(control.getURL()).to.equal('http://a.tiles.bigemap.com/geocoding/v5/bigemap.places/{query}.json?access_token=key');
    });

    it('supports custom access token', function() {
        var control = L.bigemap.geocoderControl('bigemap.places', {accessToken: 'custom'});
        expect(control.getURL()).to.equal('http://a.tiles.bigemap.com/geocoding/v5/bigemap.places/{query}.json?access_token=custom');
    });
    ...
    it('#setID', function() {
        var control = L.bigemap.geocoderControl('bigemap.places');
        expect(control.setID('bigemap.places')).to.eql(control);
        expect(control.getURL()).to.equal('http://a.tiles.bigemap.com/geocoding/v5/bigemap.places/{query}.json?access_token=key');
    });
    ...
    describe('#keepOpen', function(done) {
        it('true', function() {
            var map = new L.Map(document.createElement('div'));
            var control = L.bigemap.geocoderControl('http://example.com/{query}.json', {
                keepOpen: true
            }).addTo(map);
            expect(control._container.className).to.eql('leaflet-control-bigemap-geocoder leaflet-bar leaflet-control active');
        });
        it('false', function() {
            var map = new L.Map(document.createElement('div'));
            var control = L.bigemap.geocoderControl('http://example.com/{query}.json').addTo(map);
            expect(control._container.className).to.eql('leaflet-control-bigemap-geocoder leaflet-bar leaflet-control');
        });
    });
    ...
    describe('events', function() {
        ...
        beforeEach(function() {
            ...
            control = L.bigemap.geocoderControl('http://example.com/{query}.json', {
                proximity: false
            }).addTo(map);
        });
