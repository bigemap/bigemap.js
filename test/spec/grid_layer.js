describe('L.bigemap.gridLayer', function() {
    'use strict';
    ...
    describe('constructor', function() {
        it('is initialized', function() {
            var layer = L.bigemap.gridLayer();
            expect(layer).to.be.ok();
            expect(layer instanceof L.bigemap.GridLayer).to.be.ok();
        });

        it('is initialized with tilejson', function() {
            var layer = L.bigemap.gridLayer(helpers.tileJSON);
            expect(layer).to.be.ok();
            expect(layer.getTileJSON()).to.be.eql(helpers.tileJSON);
        });

        it('loads TileJSON from a URL', function(done) {
            var layer = L.bigemap.gridLayer('http://a.tiles.bigemap.com/v3/bigemap.map-0l53fhk2.json');
        ...
        it('loads TileJSON from an ID', function(done) {
            var layer = L.bigemap.gridLayer('bigemap.map-0l53fhk2');
        ...
        it('supports custom access token', function(done) {
            var layer = L.bigemap.gridLayer('bigemap.map-0l53fhk2', {accessToken: 'custom'});
    ...
    describe('#getData', function() {
        ...
        beforeEach(function() {
            ...
            layer = L.bigemap.gridLayer({grids: ['{z}/{x}/{y}']})
                .addTo(map);
        });
    ...
    describe('tile loading', function() {
        ...
        it('requests tiles for the current view', function() {
            ...
            L.bigemap.gridLayer({grids: ['{z}/{x}/{y}']})
                .addTo(map);
        ...
        it('requests no tiles for zooms less than the minimum', function() {
            ...
            L.bigemap.gridLayer({grids: ['{z}/{x}/{y}'], minzoom: 1})
                .addTo(map);
        ...
        it('requests no tiles for zooms greater than the maximum', function() {
            ...
            L.bigemap.gridLayer({grids: ['{z}/{x}/{y}'], maxzoom: 14})
                .addTo(map);
        ...
        it('requests no tiles outside of bounds', function() {
            ...
            L.bigemap.gridLayer({grids: ['{z}/{x}/{y}'], bounds: [-10,-10,-5,-5]})
                .addTo(map);
