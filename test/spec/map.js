describe('L.bigemap.map', function() {
    ...
    describe('constructor', function() {
        it('loads TileJSON from a URL', function(done) {
            var map = L.bigemap.map(element, 'http://a.tiles.bigemap.com/v3/bigemap.map-0l53fhk2.json');
        ...
        it('preserves manually-set marker layer GeoJSON', function() {
            ...
            server.respondWith("GET", "http://a.tiles.bigemap.com/v3/bigemap.map-0l53fhk2.json",
                [200, { "Content-Type": "application/json" }, JSON.stringify(helpers.tileJSON)]);
            server.respondWith("GET", "http://a.tiles.bigemap.com/v3/examples.map-8ced9urs/markers.geojson",
        ...
        it('passes tileLayer options to tile layer', function() {
            var map = L.bigemap.map(element, 'bigemap.map-0l53fhk2', {tileLayer: {detectRetina: true}});
            expect(map.tileLayer.options.detectRetina).to.equal(true);
        });

        it('passes featureLayer options to feature layer', function() {
            var filter = function() { return true; },
                map = L.bigemap.map(element, 'bigemap.map-0l53fhk2', {featureLayer: {filter: filter}});
    ...
    describe('map feedback support', function() {
        function improveMapHash(map) {
            return map.getContainer().getElementsByClassName('bigemap-improve-map')[0].hash;
        }

        it('adds mapid and coordinates to attribution link', function() {
            var map = L.bigemap.map(element, helpers.tileJSON_improvemap)
        ...
        it('adds mapid and coordinates to attribution link (async)', function(done) {
            var map = L.bigemap.map(element, 'examples.h8e9h88l')
                .setView([38.902, -77.001], 13);
        ...
        it('updates coordinates across layer adds/removes', function(done) {
            ...
            var layer = L.bigemap.tileLayer('examples.h8e9h88l')
                .addTo(map)
                .on('ready', function() {
                    expect(improveMapHash(map)).to.eql('#examples.h8e9h88l/-77.001/38.902/13');
