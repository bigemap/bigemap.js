describe('L.bigemap.legendControl', function () {
  ...
  describe('#addLegend', function () {
    ...
    it('adds a map legend element to its container', function () {
      ...
      legend.addTo(map);
      expect(legend.addLegend('foo')).to.eql(legend);
      expect(legend._container.innerHTML).to.eql(
        '<div class="map-legend wax-legend">foo</div>'
      );
    });
    ...
    it('handles multiple legends', function () {
      ...
      expect(legend._container.innerHTML).to.eql(
        '<div class="map-legend wax-legend">foo</div><div class="map-legend wax-legend">bar</div>'
      );
    });
  });
  ...
  it('sanitizes its content', function () {
    ...
    expect(legend._container.innerHTML).to.eql(
      '<div class="map-legend wax-legend"></div>'
    );
  });

  it('supports a custom sanitizer', function () {
    var map = L.map(document.createElement('div'));
    var legend = L.bigemap
      .legendControl({
    ...
    expect(legend._container.innerHTML).to.eql(
      '<div class="map-legend wax-legend"><script></script></div>'
    );
  });
});
