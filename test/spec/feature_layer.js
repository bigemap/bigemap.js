describe('L.bigemap.featureLayer', function() {
    ...
    it('loads data from a GeoJSON source', function() {
        ...
        expect(layer instanceof L.bigemap.FeatureLayer).to.be.equal(true);
        expect(marker.getLatLng()).to.be.near({lng: -77.0203, lat: 38.8995}, 0);
    });

    it('loads data from a GeoJSON URL', function() {
        var url = 'http://api.tiles.bigemap.com/v4/examples.map-zr0njcqy/features.json',
    ...
    it('loads data for a map ID', function() {
        ...
        server.respondWith("GET", internals.url('/v4/bigemap.map-0l53fhk2/features.json'),
            [200, { "Content-Type": "application/json" }, JSON.stringify(helpers.geoJson)]);
        server.respond();
    ...
    it('supports custom access token', function() {
        var layer = L.bigemap.featureLayer('bigemap.map-0l53fhk2', {accessToken: 'custom'});

        server.respondWith("GET", internals.url('/v4/bigemap.map-0l53fhk2/features.json', 'custom'),
            [200, { "Content-Type": "application/json" }, JSON.stringify(helpers.geoJson)]);
        server.respond();
    ...
    describe("#loadURL", function() {
        it('returns self', function() {
            ...
            expect(layer.loadURL('http://a.tiles.bigemap.com/v3/bigemap.map-0l53fhk2.json')).to.eql(layer);
        });
        ...
        it('emits a ready event', function(done) {
            ...
            layer.loadURL('http://a.tiles.bigemap.com/v3/bigemap.map-0l53fhk2.json');
    ...
    describe("#setGeoJSON", function() {
        ...
        it('supports custom access token', function() {
            var layer = L.bigemap.featureLayer(undefined, {accessToken: 'custom'})
