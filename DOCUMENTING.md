Examples and API documentation for Bigemap.js live in the `docs` subdirectory.
...
* Examples use hosted bigemap.js, with the latest deployed version.
...
```sh
git clone git@github.com:bigemap/bigemap.js.git
cd bigemap.js
make
jekyll serve -w -p 4000
```

Edit the site. Existing examples [are located here](https://github.com/bigemap/bigemap.js/tree/mb-pages/docs/_posts/examples/v1.0.0) -
copy one to a new file with the same naming convention to start a new example. Test your new example
by going to http://localhost:4000/bigemap.js/ and finding and using it.
...
## Updating the site to the latest deployed version of bigemap.js:
...
## To update www.bigemap.com to use the latest version
Bump the `bigemapjs` version field in
* `_config.yml`
* `_config.master.yml`
* `_config.test.yml`
